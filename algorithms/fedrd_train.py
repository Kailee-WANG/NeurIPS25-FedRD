...
from utils.fed_merge import Cal_Weight_Dict, FedAvg, FedUpdate, Global_Aggregation
from utils.trainval_func import site_evaluation, site_gap_evaluation, site_trainrd, GetFedModel, SaveCheckPoint
...
def get_argparse():
    ...
    parser.add_argument('--fs_classes', type=str, default="p:4,5,6;a:0,1,6;c:3,4,6;s:1,2,3",
                       help="Format: 'domain1:cls1,cls2;domain2:cls1,cls2'")
    parser.add_argument('--indicator', type=str, default='layer1.1.conv2.weight',
                      help="Name of the target layer")
...
def main():
    ...
    file_name = 'fedrd_'+os.path.split(__file__)[1].replace('.py', '')
    args = get_argparse()
    ...
    weight_dict = {}
    global_local_gap = {}
    for item in args.fs_classes.split(';'):
        domain, classes = item.split(':')
        fs_classes[domain] = [int(cls) for cls in classes.split(',')]
    for site_name in dataobj.train_domain_list:
        weight_dict[site_name] = 1./3.
        global_local_gap[site_name] = None
    ...
    for i in range(args.comm+1):
        ...
        for domain_name in dataobj.train_domain_list:
            site_trainrd(i, domain_name, args, model_dict[domain_name], optimizer_dict[domain_name], 
                       scheduler_dict[domain_name],dataloader_dict[domain_name]['train'], log_ten, metric,fs_classes=fs_classes, global_model=global_model)
            
            global_local_gap[domain_name] = site_gap_evaluation(i, domain_name, args, model_dict[domain_name], global_model, dataloader_dict[domain_name]['val'], log_file, log_ten, metric, note='gap')
        
        weight_dict = Global_Aggregation(dataset_dict, model_dict, global_model, args, site_list=dataobj.train_domain_list, global_local_gap=global_local_gap)
