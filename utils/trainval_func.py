...
def compute_lambda(local_model, global_model):
    local_m = local_model.state_dict()
    global_m = global_model.state_dict()
    local_weight = local_m['fc_class.weight']
    global_weight = global_m['fc_class.weight']
    lambda_val = torch.norm(global_weight - local_weight, dim=1).mean()
...
def epoch_site_trainrd(epochs, site_name, args, model, optimzier, scheduler, dataloader, log_ten, metric, fs_classes, global_model):
    model.train()
    lambda_val = compute_lambda(model, global_model)
...
def site_trainrd(comm_rounds, site_name, args, model, optimizer, scheduler, dataloader, log_ten, metric, fs_classes, global_model):
    tbar = tqdm(range(args.local_epochs))
    for local_epoch in tbar:
        tbar.set_description(f'{site_name}_train')
        epoch_site_trainrd(comm_rounds*args.local_epochs + local_epoch, site_name, args, model, optimizer, scheduler, dataloader, log_ten, metric, fs_classes, global_model)
...
def site_gap_evaluation(epochs, site_name, args, model, global_model, dataloader, log_file, log_ten, metric, note='gap'):

    local_dict = site_evaluation(epochs, site_name, args, model, dataloader, log_file, log_ten, metric, note='local')
    global_dict = site_evaluation(epochs, site_name, args, global_model, dataloader, log_file, log_ten, metric, note='global')
    site_gap= global_dict['loss'] - local_dict['loss'] 
...
def GetFedModel(args, num_classes, is_train=True):
    ...
    for domain_name in domain_list:
        ...
        optimizer_dict[domain_name] = torch.optim.SGD(model_dict[domain_name].parameters(), lr=args.lr, momentum=0.9,
                                                      weight_decay=5e-4)
        #optimizer_dict[domain_name] = torch.optim.Adam(model_dict[domain_name].parameters(),lr=args.lr, betas=(0.9, 0.999),eps=1e-08,  weight_decay=5e-4)
