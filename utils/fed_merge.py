...
def Global_Aggregation(dataset_dict, model_dict, global_model, args, site_list=None, global_local_gap=None):
    if site_list is None:
        site_list = list(dataset_dict.keys())
    d_dict = {}
    global_param = global_model.state_dict()[args.indicator]
    for site_name, local_model in model_dict.items():
        local_param = local_model.state_dict()[args.indicator]
        d_i = torch.norm(global_param - local_param, p=2).item()
        d_dict[site_name] = d_i
    total_d = sum(d_dict.values())
    beta_dict = {}
    if total_d > 1e-8:
        for site in site_list:
            beta_dict[site] = d_dict[site]/total_d
    else:
        for site in site_list:
            beta_dict[site] = 1.0/len(model_dict)
    gamma_dict = {}
    for site in site_list:
        gap = global_local_gap[site]
        gamma_dict[site] = 1 / (1 + np.exp(-gap))
    sum_gamma = sum(gamma_dict.values())
    weight_dict = {}
    for site in site_list:
        term1 = (1 - beta_dict[site]) / (len(model_dict) - 1) 
        if sum_gamma > 0:
            term2 = gamma_dict[site] / sum_gamma
        else:
            term2 = len(model_dict)
        weight_dict[site] = 0.5 * (term1 + term2)
